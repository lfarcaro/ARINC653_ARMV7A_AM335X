...
// FL translation table pointer
static unsigned int *PTR_FLTRANSLATIONTABLE_POINTER;

// FL translation table size
static unsigned int SIZ_FLTRANSLATIONTABLE_SIZE;

// FL translation table full size
static unsigned int SIZ_FLTRANSLATIONTABLE_FULL;

// SL translation table pointer
static unsigned int *PTR_SLTRANSLATIONTABLE_POINTER;

// SL translation table size
static unsigned int SIZ_SLTRANSLATIONTABLE_SIZE;

// SL translation table full size
static unsigned int SIZ_SLTRANSLATIONTABLE_FULL;
...
unsigned char MMU_FLTRANSLATIONTABLE_ALLOCATE(unsigned int **PTR_FLTRANSLATIONTABLE) {
	...
	// Verifies full size
	if (SIZ_FLTRANSLATIONTABLE_FULL + (MMU_FLTRANSLATIONTABLE_ENTRYCOUNT * MMU_FLTRANSLATIONTABLE_ENTRYSIZE) - 1 >= SIZ_FLTRANSLATIONTABLE_SIZE) {
		return 0;
	}

	// Fills FL translation table with fault entries
	for (PTR_CLEAR = PTR_FLTRANSLATIONTABLE_POINTER; PTR_CLEAR < (PTR_FLTRANSLATIONTABLE_POINTER + MMU_FLTRANSLATIONTABLE_ENTRYCOUNT); PTR_CLEAR++) {
		*PTR_CLEAR = 0x00000000;
	}

	// Prepares address
	*PTR_FLTRANSLATIONTABLE = PTR_FLTRANSLATIONTABLE_POINTER;

	// Moves pointer
	PTR_FLTRANSLATIONTABLE_POINTER += MMU_FLTRANSLATIONTABLE_ENTRYCOUNT;

	// Calculates new full size
	SIZ_FLTRANSLATIONTABLE_FULL += (MMU_FLTRANSLATIONTABLE_ENTRYCOUNT * MMU_FLTRANSLATIONTABLE_ENTRYSIZE);
	return 1;
}
...
unsigned char MMU_SLTRANSLATIONTABLE_ALLOCATE(unsigned int **PTR_SLTRANSLATIONTABLE) {
	...
	// Verifies full size
	if (SIZ_SLTRANSLATIONTABLE_FULL + (MMU_SLTRANSLATIONTABLE_ENTRYCOUNT * MMU_SLTRANSLATIONTABLE_ENTRYSIZE) - 1 >= SIZ_SLTRANSLATIONTABLE_SIZE) {
		return 0;
	}

	// Fills SL translation table with fault entries
	for (PTR_CLEAR = PTR_SLTRANSLATIONTABLE_POINTER; PTR_CLEAR < (PTR_SLTRANSLATIONTABLE_POINTER + MMU_SLTRANSLATIONTABLE_ENTRYCOUNT); PTR_CLEAR++) {
		*PTR_CLEAR = 0x00000000;
	}

	// Prepares address
	*PTR_SLTRANSLATIONTABLE = PTR_SLTRANSLATIONTABLE_POINTER;

	// Moves pointer
	PTR_SLTRANSLATIONTABLE_POINTER += MMU_SLTRANSLATIONTABLE_ENTRYCOUNT;

	// Calculates new full size
	SIZ_SLTRANSLATIONTABLE_FULL += (MMU_SLTRANSLATIONTABLE_ENTRYCOUNT * MMU_SLTRANSLATIONTABLE_ENTRYSIZE);
	return 1;
}
...
void MMU_STARTUP(unsigned int *PTR_FLTRANSLATIONTABLE_REGION_POINTER, unsigned int SIZ_FLTRANSLATIONTABLE_REGION_SIZE, unsigned int *PTR_SLTRANSLATIONTABLE_REGION_POINTER, unsigned int SIZ_SLTRANSLATIONTABLE_REGION_SIZE) {
	...
	PTR_FLTRANSLATIONTABLE_POINTER = PTR_FLTRANSLATIONTABLE_REGION_POINTER;
	SIZ_FLTRANSLATIONTABLE_SIZE = SIZ_FLTRANSLATIONTABLE_REGION_SIZE;
	SIZ_FLTRANSLATIONTABLE_FULL = 0;

	// Initializes SL translation table region
	PTR_SLTRANSLATIONTABLE_POINTER = PTR_SLTRANSLATIONTABLE_REGION_POINTER;
	SIZ_SLTRANSLATIONTABLE_SIZE = SIZ_SLTRANSLATIONTABLE_REGION_SIZE;
	SIZ_SLTRANSLATIONTABLE_FULL = 0;
	...
	// Disables TEX remapping and access flag usage
	CP15ControlFeatureDisable(CP15_CONTROL_TEXREMAP);

	// Enables alignment check
	CP15ControlFeatureEnable(CP15_CONTROL_ACCESSFLAG | CP15_CONTROL_ALIGN_CHCK);
}
...
void MMU_ENABLE(unsigned int N, unsigned int *PTR_FLTRANSLATIONTABLE0, unsigned int *PTR_FLTRANSLATIONTABLE1, unsigned char CHR_CONTEXTID) {
	...
	// Sets TTB0
	CP15Ttb0Set((unsigned int) PTR_FLTRANSLATIONTABLE0);

	// Sets TTB1
	CP15Ttb1Set((unsigned int) PTR_FLTRANSLATIONTABLE1);

	// Invalidates TLB entries
	CP15TlbInvalidate();

	// Sets context identifier
	CP15SetContextID((((unsigned int) CHR_CONTEXTID) << 8) | ((unsigned int) CHR_CONTEXTID));
